...
### 新增
- `ConditionalForecastBlock`：每支条件默认两行——行1 `方向徽+路径名`（右侧纯文字"详情 ▾"右对齐）、行2 `若[触发关键词] 则[预判标签]`；点行1"详情"在行3展开完整预判句（浅块、幅度灰），命中徽时行1让出右侧
- 类型补 `scenario_keywords?`（scenario 摘要，方向+幅度，2026-09-03 起新数据携带）；预判标签优先该字段，旧数据回退 scenario 幅度段
- `dev/App.vue` 待验证卡补 scenario_keywords 示例（站稳平台 +3% 等）

### 兜底
- sentence 模式 / 无 label / 无预判标签 → 整句原文直显不失真

---

## [main] 2026-09-03 — 预判卡情景行换行优化（灰幅度段与黑字粘连）

**开发者**: Aria

### 改进
- `ConditionalForecastBlock` 情景正文由 flex-chip 拆分改**整句内联文本流**：灰色幅度段（+3%~+5% 等）不再独立拆行，首尾插入零宽连字符 U+2060 与前后文字粘连，行内自然换行不孤立断句
- 幅度识别正则补 `±`（`波幅小于±1%` 整体灰化）
